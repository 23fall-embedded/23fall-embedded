...
def test(loc="威海"):
    while True:
        ...
        if message["code"] == "200":
            now = message["now"]
            ssd3306.show2(
                f"现在天气状况：{now['text']}，温度：{now['temp']}°C，体感温度：{now['feelsLike']}°C，湿度：{now['humidity']}%，风向风力：{now['windDir']}{now['windScale']}级",
                "，",
                9,
            )
        else:
        ...
        message = get_req(url)
        print(message["fxLink"])
        if message["code"] == "200":
            tomorrow = message["daily"][1]
            day2 = message["daily"][2]
            ssd3306.show2(
                f"明天白天天气状况：{tomorrow['textDay']}，温度：{tomorrow['tempMin']}到{tomorrow['tempMax']}°C，湿度：{tomorrow['humidity']}%，风向风力：{tomorrow['windDirDay']}{tomorrow['windScaleDay']}级",
                "，",
                9,
            )
            ssd3306.show2(
                f"明天夜间天气状况：{tomorrow['textNight']}，温度：{tomorrow['tempMin']}到{tomorrow['tempMax']}°C，风向风力：{tomorrow['windDirNight']}{tomorrow['windScaleNight']}级",
                "，",
                9,
            )
            ssd3306.show2(
                f"后天白天天气状况：{day2['textDay']}，温度：{day2['tempMin']}到{day2['tempMax']}°C，湿度：{day2['humidity']}%，风向风力：{day2['windDirDay']}{day2['windScaleDay']}级",
                "，",
                9,
            )
            ssd3306.show2(
                f"后天夜间天气状况：{day2['textNight']}，温度：{day2['tempMin']}到{day2['tempMax']}°C，风向风力：{day2['windDirNight']}{day2['windScaleNight']}级",
                "，",
                9,
            )
        else:
